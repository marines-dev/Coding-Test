// arraylist.cpp : 이 파일에는 'main' 함수가 포함됩니다. 거기서 프로그램 실행이 시작되고 종료됩니다.
//
...
void DisplayArrayList(ArrayList* _pList)
{
	int i = 0;
	if (_pList != NULL)
	{
		printf("최대 원소 개수 : %d\n", _pList->maxElementCount);
		printf("현재 원소 개수 : %d\n", _pList->currentElementCount);

		for (i = 0; i < _pList->currentElementCount; ++i)
		{
			printf("Index[%d] : %d\n", i, GetElement(_pList, i)->data);
		}

		i = _pList->currentElementCount;
		for (; i < _pList->maxElementCount; ++i)
		{
